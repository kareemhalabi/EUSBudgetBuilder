      ...
      if (cell.getCellType() == Cell.CELL_TYPE_STRING)
      {
         for (String pastEndRef : pastEndRefs)
         {
            ...
            if (strValue != null && strValue.indexOf(pastEndRef) >= 0)
            {
               ...
               // If we have no variables use legacy behavior
               if(strParts == null || strParts.length == 0) {
                  takeAction = true;
                  break;
               }
               else {
                  // Check to see if we have multiple variables in the cell
                  // and at least one variable is not in the pastEnd list
                  boolean isExpressionPastEnd = true;
                  for(String expression : strParts) {
                     // Discard validating part so the cell text that is not
                     // part of a variable
                     if(expression.indexOf("}") >= 0) {
                        isExpressionPastEnd = false;
                        
                        // Now that we have an expression, we will see if it
                        // is in the past end list
                        for (String pastEndRef2 : pastEndRefs) {
                           if(expression.indexOf(pastEndRef2) >= 0) {
                              isExpressionPastEnd = true;
                              break;
                           }
                        }
                        // The expression is not past end so let the cell be
                        // processed
